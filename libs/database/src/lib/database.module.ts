  ...
  imports: [
    TypeOrmModule.forRootAsync({
      useFactory: async () => {
        ...
        const result = await conn.query(`SHOW DATABASES LIKE '${process.env.MYSQL_DB || 'rail'}';`);
        const shouldSync = ((result as unknown[]).length < 1) || process.env.FORCE_SYNC_DB != null;
        if ((result as unknown[]).length == 0) {
          await conn.query(
            `CREATE DATABASE IF NOT EXISTS ${process.env.MYSQL_DB || 'rail'}`
          );
        }
        return {
          ...baseConn,
          database: process.env.MYSQL_DB || 'rail',
