...
import { GoogleServicesService } from '@rail/order/google-services/google-services.service';
...
export class DriverOrderQueryService extends TypeOrmQueryService<RequestEntity> {
    ...
    async updateOne(id: number, update: UpdateOrderInput): Promise<RequestEntity> {
        let order = await this.orderRepository.findOne(id, { relations: ['rider', 'service', 'options']});
        switch (update.status) {
            ...
            case OrderStatus.Finished: {
                ...
                order = await this.orderRepository.findOne(id, { relations: ['rider', 'service', 'options']});
