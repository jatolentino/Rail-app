...
import { CryptoService } from '@rail/database';
...
export class DriverAPIController {
    ...
    @Get('payment_result')
    async verifyPayment(@Req() req: fastify.FastifyRequest<{Querystring: {token: string}}>, @Res() res: fastify.FastifyReply) {
    ...
    @Get('success_message')
    async successMessage(@Req() req: fastify.FastifyRequest<{ Querystring: { id_order: string } , Body: { posted_data: string } }>, @Res() res: fastify.FastifyReply) {
        res.send('Transaction successful. Close this page and go back to the app.');
    }
    ...
    async upload(@Request() req: any, @Res() res: fastify.FastifyReply) {
        ...
        await this.driverRepository.update((req as unknown as any).user.id, {mediaId: insert.raw.insertId });
    ...
    async uploadDocuement(@Request() req: any, @Res() res: fastify.FastifyReply) {
        ...
        const _fileName = join(dir, `${new Date().getTime()}-${data.filename}`);
        await pump(data.file, createWriteStream(_fileName));
        const insert = await this.mediaRepository.insert({ address: _fileName, driverDocumentId: (req as unknown as any).user.id });
        res.code(200).send({ id: insert.raw.insertId, address: _fileName });
    }
}
