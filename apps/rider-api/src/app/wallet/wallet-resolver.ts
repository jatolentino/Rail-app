...
import { PaymentGatewayEntity } from '@rail/database/payment-gateway.entity';
...
export class WalletResolver {
  ...
  async topUpWallet(@Args('input', { type: () => TopUpWalletInput }) input: TopUpWalletInput): Promise<TopUpWalletResponse> {
    ...
    const params = `userType=client&userId=${this.context.req.user.id}&paymentGatewayId=${gateway.id}&amount=${input.amount}&currency=${input.currency}&returnUrl=${process.env.RIDER_SERVER_URL}/payment_result`
    return {
