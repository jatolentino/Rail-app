...
import 'package:rail/order_invoice_view.dart';
import 'package:rail/ride_options_sheet_view.dart';
import 'package:rail/rider_preferences_sheet_view.dart';
                              ...
                              if (order.status == OrderStatus.driverAccepted)
                                Timeago(
                                    builder: (context, text) {
                                      return Text(
                                        "Rider ${(order.etaPickup?.isBefore(DateTime.now()) ?? false) ? "Expected you" : "Expects you in"} $text",
                                        style: Theme.of(context)
                                            .textTheme
                                            .labelSmall,
                                      );
  ...
  String getTitleForStatus(OrderStatus status) {
    switch (status) {
      case OrderStatus.driverAccepted:
        return "Rider will be notified once you tap Arrived";

      case OrderStatus.arrived:
        return "Rider has been notified";
