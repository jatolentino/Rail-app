...
import { CreateServiceGQL, NewServiceQuery, ServicePaymentMethod, SetRegionsOnServiceGQL, UpdateServiceGQL, ViewServiceQuery } from '@rail/admin-panel/generated/graphql';
import { RouterHelperService } from '@rail/admin-panel/src/app/@services/router-helper.service';
import { environment } from '@rail/admin-panel/src/environments/environment';
...
export class ManagementServicesViewComponent implements OnInit {
  ...
  jwt = localStorage.getItem('rail_admin_token');
  avatarUrl?: string;
  ...
  ngOnInit(): void {
    ...
    this.route.data.subscribe(data => {
      ...
      if(data.service.data.service.regions.length > 0) {
        ...
        this.formatterPrice = (value: number) => (value != null ? `${value} ${currency}` : '');
      }
